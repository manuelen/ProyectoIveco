...
$resultado=mysql_query("SELECT dispositivos.ip,dispositivos.nombre as nombre, count( * ) AS cantidad FROM `controldeerrores` inner join dispositivos on dispositivos.ip = controldeerrores.ip WHERE puerto = '' AND (controldeerrores.fecha BETWEEN '$fechaini' and '$fechafin')  GROUP BY dispositivos.ip ORDER BY cantidad DESC limit 5");
$fila=mysql_fetch_array($resultado);

						for($i=0;$i<$fila;$i++)
		{                
$intTotalAnio1[$i] = $fila['cantidad'];
...
$strXML = "<chart caption = 'Servidores caídos' bgColor='#CDDEE5' baseFontSize='10' showValues='1' xAxisName='Servidores' >";
